...
public class GaussPageRankReducer extends MapReduceBase implements
	...
	public void reduce(Text key, Iterator<Text> values,
		...
		float blockResidualError;
		do {
			blockResidualError = IteratorBlockOnce();
			counter++;
		} while (blockResidualError >= GaussPageRank.ERROR_BOUND);
		...
		for (Integer node : nodeList) {
			...
			reporter.getCounter(GaussCounter.RESIDUAL_COUNTER).increment((long)(residualError * GaussPageRank.MULTIPLE));
			if (node == maxNode) {
				reporter.incrCounter("BlockCounter", "PAGERANK" + Integer.parseInt(key.toString()), 
						(long) (newPageRanks.get(node) * GaussPageRank.MULTIPLE));
			}
		}
		
		// increase reducer counter and inner iteration counter
		reporter.getCounter(GaussCounter.REDUCER_COUNTER).increment(1);
		reporter.getCounter(GaussCounter.INNER_ITERATION_COUNTER).increment(counter);
		...
		for (Integer node : nodeList) {
			...
			if (beMap.containsKey(node)) {
				...
				for (Integer comingNode : beMap.get(node)) {
					// NPR[v] += PR[u] / deg(u);
					newPageRank += (float) newPageRanks.get(comingNode) / nodeDegrees.get(comingNode);
				}
			}
			if (bcMap.containsKey(node)) {
				...
				for (Float pageRankFactor : bcMap.get(node)) {
					// NPR[v] += R;
					newPageRank += pageRankFactor;
				}
			}
			// NPR[v] = d*NPR[v] + (1-d)/N;
			newPageRank = DAMPING_FACTOR * newPageRank + (1 - DAMPING_FACTOR) / GaussPageRank.NUMBER_OF_NODES;
